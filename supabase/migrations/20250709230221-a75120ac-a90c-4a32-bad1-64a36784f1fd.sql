-- Complete fix for ROUND function type issues
-- This migration will completely rebuild the dashboard functions with proper type handling
...
-- Recreate get_dashboard_metrics with correct parameter order and numeric casting
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            THEN (COALESCE(SUM(s.price), 0)::numeric / COALESCE(SUM(s.comission + s.retention + s.shipping), 1)::numeric)
            ELSE 0::numeric
        END as mer,
        CASE 
            WHEN COUNT(DISTINCT s.id) > 0 
            THEN (COALESCE(SUM(s.price), 0)::numeric / COUNT(DISTINCT s.id)::numeric)
            ELSE 0::numeric
        END as aov,
        ...
        CASE 
            ...
            THEN ROUND((COALESCE(SUM(s."Profit"), 0)::numeric / COALESCE(SUM(s.price), 1)::numeric * 100), 2)
            ELSE 0::numeric
...
-- Recreate get_channel_metrics with correct parameter order and numeric casting
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN COUNT(DISTINCT s.id) > 0 
            THEN ROUND((COALESCE(SUM(s.price), 0)::numeric / COUNT(DISTINCT s.id)::numeric), 2)
            ELSE 0::numeric
        END as aov,
        COALESCE(SUM(s."Profit"), 0)::numeric as contribution_margin,
        CASE 
...
-- Recreate get_sales_chart_data with correct parameter order
...
-- Recreate get_channel_distribution with correct parameter order and numeric casting
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN total_sales > 0 
            THEN ROUND((COALESCE(SUM(s.price), 0)::numeric / total_sales * 100), 1)
            ELSE 0::numeric
        END as percentage
    FROM "Sales" s
    WHERE s.company_id = ANY(user_company_ids)
